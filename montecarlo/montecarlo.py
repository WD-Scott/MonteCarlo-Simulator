...
class Die:
    ...
    def __init__(self, faces):
        '''
        Initializer that takes an array or list of faces of any length, 
        initializes weights to 1, then saves to a private dataframe.
        ...
        self.faces_weights = pd.DataFrame(data = faces, columns = ['Faces'], index = range(len(faces)))
        self.faces_weights['Weights'] = np.array([1.0 for i in range(len(faces))])
    ...
    def change_weight(self, face, new_weight):
        ...
        '''
        if not (face in list(self.faces_weights['Faces'])):
            return ValueError ("That face does not exist.")
        else:
            self.faces_weights.loc[self.faces_weights['Faces'] == face, 'Weights'] = float(new_weight)
    ...
    def rolls(self, rolls=1):
        ...
        '''
        rolled_outcome = self.faces_weights.sample(n = rolls, weights = 'Weights', replace = True)
...
class Game:
    ...
    def play(self, rolls):
        ...
        Results in a table of data with columns for: 
            roll number
